    ...
    def _load_data_to_memory(self):
        ...
        for parent, _, files_list in os.walk(self.img_dir):
            ...
            for img_file_name in files_list:
                ...
                try:
                    with open(os.path.join(self.annotation_dir, parent_path, annotation_file_name), 'r') as f:
                        annotations = f.read().split('\n')
                        self.imgs.append(rel_img_path)
                        truth[rel_img_path] = []
                        for ant in annotations:
                            truth[rel_img_path].append([int(float(j)) for j in ant.split(',')])
